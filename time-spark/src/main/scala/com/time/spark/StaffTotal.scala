...
object StaffTotal {
  def main(args: Array[String]) {
    ...
    val checkInData = logData.filter(line=>line.contains("account"))
    val rawRDD = checkInData.map(line=>(line.split("account: ").last.split(", args:").head,1))
    val staffCounts = rawRDD.reduceByKey((a,b)=> a+b)
    println("staffCounts === " + staffCounts.count())
